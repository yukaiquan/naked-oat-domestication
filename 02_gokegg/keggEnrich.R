...
p <- ggplot(epk, aes(y = Description, x = GeneRatio_num, fill = p.adjust)) +
    geom_bar(stat = "identity", width = 0.8) + # Bar chart width setting
    scale_fill_gradient(low = "red", high = "blue") +
    labs(
        title = "KEGG Pathways Enrichment", # Set title, x-axis and y-axis names
        x = "Gene Ratio",
        y = "Pathway"
    ) +
    theme(
...
# Change column names of ekp_results
